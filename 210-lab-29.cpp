...
void runSimulation(map<string, array<list<string>, SIZE> >& storeData, int intervals) {
	...
	for (int i = 0; i < intervals; i++) {
		...
		if (eventOccurs(50)) {
			...
			while (!found && it != storeData.at("shop1")[0].end()) {
				if (*it == order) { // If the flowers needed for the customer's order are available, remove the flowers from the shop's inventory
					cout << "\tCustomer ordered 1 " << order << " and 1 " << *it << " was removed from the store's inventory." << endl;
			...
			if (!found) {
				...
				cout << "\tCustomer ordered 1 " << order << " and it was not in stock. A request has been made to the supplier and the customer has been added to the queue" << endl;
		...
		if (eventOccurs(75) && !storeData.at("shop1")[1].empty()) {
			anyEventOccured = true;
